...
def TT(): return int(str(time.time()*1000).split('.')[0])
...
class loginClient(object):
    def __init__(self, debug=False):
        ...
        self.cookies = {}
        # for requests "requests.get(url = url, data = data, headers = headers, cookies = self.cookies)"

        self.debug = debug

    def prelogin(self):
        url = r"https://login.sina.com.cn/sso/prelogin.php?entry=weibo&callback=sinaSSOController.preloginCallBack" \
            r"&su=&rsakt=mod&client=ssologin.js(v1.4.19)&_=%s" % TT()
        html = requests.get(url, headers=self.headers).text

        jsonStr = jsonStr = re.findall(r'({.*?})', html)[0]
    ...
    def send_privateMessage(self, token):
        ...
        send_privateMessage_response = self.openner.open(
            send_privateMessage_request)
        send_privateMessage_response = json.loads(
            send_privateMessage_response.read().decode('utf8'))
        if self.debug:
            print('send_privateMessage_response =',
                  send_privateMessage_response)
    ...
    def check_approvalStatus(self, token):
        ...
        check_approvalStatus_response = self.openner.open(
            check_approvalStatus_request)
        check_approvalStatus_response = json.loads(
            check_approvalStatus_response.read().decode('utf8'))
        if self.debug:
            print('check_approvalStatus_response =',
                  check_approvalStatus_response)
    ...
    def request_final(self, ticket, ssosavestate):
        final_url = 'https://passport.weibo.com/wbsso/login?ticket='
        final_url += ticket + '&ssosavestate=' + ssosavestate
        final_url += '&callback=sinaSSOController.doCrossDomainCallBack&scriptId=ssoscript0&client=ssologin.js(v1.4.19)'
    ...
    def main(self, username, password):
        ...
        crossdomain1_url = check_approvalStatus_response['data']['redirect_url'][:-
                                                                                 26] + 'http%3A%2F%2Fmy.sina.com.cn'
        if self.debug:
            print('crossdomain1_url =', crossdomain1_url)

        crossdomain1_response_text = self.request_crossdomain1(
            crossdomain1_url)

        pattern = r'https://.+?&#39;'
        crossdomain2_url = re.search(
            pattern, crossdomain1_response_text).group()[:-5]
        if self.debug:
            print('crossdomain2_url =', crossdomain2_url)

        crossdomain2_response_text = self.request_crossdomain2(
            crossdomain2_url)
        ...
        print(cookie_dict)
        cookies = "; ".join(
            cookie + "=" + cookie_dict[cookie] for cookie in cookie_dict)
        ...
        if login_result_dict['result']:
            print("Login success!")
            self.loginSuccessFlag = 1
        else:
            print("Login Failed!")
            self.loginSuccessFlag = 0
        Cookies = urllib.parse.unquote(self.cookies["Cookies"])
        self.cookies['Cookies'] = Cookies
        return self.cookies
